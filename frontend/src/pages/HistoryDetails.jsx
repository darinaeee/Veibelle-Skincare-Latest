...
const HistoryDetails = () => {
  ...
  useEffect(() => {
    // item is passed from History.jsx → navigate("/history-details", { state: item })
    if (!location.state) {
      // if user refreshed or opened directly, send back to history page
      navigate("/history");
      return;
    }
      ...
      <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="flex flex-col">
              ...
              <span
                className={`font-['Poppins'] text-lg font-semibold ${
                  data.profile.allergens?.length ? "text-red-700" : "text-gray-400"
                }`}
              >
                {data.profile.allergens?.length
                  ? data.profile.allergens.join(", ")
                  : "None"}
              </span>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {data.profile.eyeConcerns?.length > 0 ? (
                  data.profile.eyeConcerns.map((e, i) => (
                    ...
                    >
                      {e}
                      {i < data.profile.eyeConcerns.length - 1 ? ", " : ""}
                    </span>
                  ))
                ) : (
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          <div className="flex items-center justify-between border-b border-gray-200 pb-4">
            <h3 className="font-['Cinzel'] text-2xl text-[#1a1a1a] font-bold">
              Past Recommendations
            </h3>
            <span className="text-xs font-['Poppins'] text-gray-500 uppercase tracking-wide hidden md:block">
              Generated Result
            </span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {data.results && data.results.length > 0 ? (
              ...
              <div className="col-span-full py-10 text-center text-gray-400 font-['Poppins']">
                No products were recommended in this session.
              </div>
            )}
          </div>
        </div>
